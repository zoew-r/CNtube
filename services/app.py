...
from .video_processor import extract_audio_from_url # 修改3-5行 (adding a dot)
...
vocab_extractor = HybridVocabExtractor()
...
def process_task():
    try:
        ...
        if task == "transcribe":
            ...
            # 如果你也把單字卡功能併入了：
            result = vocab_extractor.get_word_card(word) 
            # 如果沒有併入，這段要註解掉，否則會 crash
            ...
            # 將 level 傳入方法中
            result = vocab_extractor.simplify_text(text, target_level=level) 
            return jsonify(result)
            ...
            # 呼叫提取器
            vocab_list = vocab_extractor.extract_vocab(text)
