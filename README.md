...
- **AI 語法分析 (RAG)**：結合 RAG 技術，根據使用者等級檢索相關文法規則。
  - 提供句子的**英文翻譯**、**注音 (Bopomofo)** 與 **漢語拼音**。
  - 分析句子中出現的文法點，並標註其所屬等級。
  - 提供詳細的英文文法解說。
...
- **Python 3.10+**
- **FFmpeg** (用於音訊處理)
- **Ollama** (用於 AI 語法分析，建議模型 `qwen2.5:7b`)
...
4. **啟動程式**
   ...
   > [!IMPORTANT]
   > **關於快取效能 (Cache Performance)**
   > 系統會在執行目錄下尋找 `faiss_index` 資料夾。如果找不到，會在啟動時**重新建立索引**，這可能需要數分鐘的時間。
   > 請務必在 `CNtube` 專案根目錄下執行程式，以免造成重複建立索引與啟動緩慢。
